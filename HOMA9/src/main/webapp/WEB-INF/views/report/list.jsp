		...
		<table class="table table-hover">
			...
			<c:forEach items="${list }" var="list">
				<tr>
					<td>${list.rp_num}</td>
					<td>
						<a href="/report/read?rp_num=${list.rp_num}&page=${scri.page}&perPageNum=${scri.perPageNum}&searchType=${scri.searchType}&keyword=${scri.keyword}">${list.rp_subject}</a>
					</td>
					<td>${list.userId }</td>
					<td>
						<fmt:formatDate value="${list.rp_date}" pattern="yyyy-MM-dd" />
					</td>
					<td>${list.rp_readCount }</td>
			</c:forEach>
			<!-- 목록끝 -->
		</table>
