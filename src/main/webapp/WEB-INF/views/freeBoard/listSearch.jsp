		...
		<table class="table table-hover">
			...
			<c:forEach items="${list }" var="list">
				<tr>
					<td>${list.fb_num}</td>
					<td>
						<a href="/freeBoard/read?fb_num=${list.fb_num}&page=${scri.page}&perPageNum=${scri.perPageNum}&searchType=${scri.searchType}&keyword=${scri.keyword}">${list.fb_subject}</a>
						<span class="badge bg-teal"><i class="glyphicon glyphicon-comment"></i>+${list.comment_cnt}</span>
					</td>
					<td>${list.userId }</td>
					<td>
						<fmt:formatDate value="${list.fb_date}" pattern="yyyy-MM-dd" />
					</td>
					<td>${list.fb_readCount }</td>
			</c:forEach>
			<!-- 목록끝 -->
		</table>
